<?php
/**
 * Class for managing the top header, settings pages, and additional features of Wbcom plugins.
 *
...
if ( ! class_exists( 'Wbcom_Admin_Settings' ) ) {
	...
	/**
	 * Class responsible for adding admin settings for Wbcom plugins.
	 *
	 * @author   Wbcom Designs
	 * @since    2.0.0
	 */
	class Wbcom_Admin_Settings {
		...
		/**
		 * Constructor for initializing admin settings.
		 *
		 * Registers shortcodes, menu pages, and admin scripts.
		 *
		 * @since 2.0.0
		 * @access public
		 */
		...
		/**
		 * Callback function for handling the extensions cards.
		 *
		 * This processes the AJAX request and returns the required data.
		 *
		 * @return void
		 */
		public function wbcom_addons_cards_links() {
			...
			if ( ! empty( $wbcom_setting_nonce ) && wp_verify_nonce( $wbcom_setting_nonce, 'wbcom_admin_setting_nonce' ) && 'wbcom_addons_cards' === $action ) {
				...
				wp_send_json( $display_extention );				
			}
		}
		 ...
		 * @return string The file path of the plugin.
		 */
		...
		/**
		 * Returns the download URL for a plugin from the WordPress repository.
		 *
		 * @since 2.0.0
		 * @access public
		 * @param string $slug The slug of the plugin.
		...
		/**
		 * Retrieves the download URL for a plugin from the WordPress repository.
		 *
		 * @since 2.0.0
		 * @access public
		 * @param string $slug The plugin's slug.
		 * @return string The download link or an error message.
		 */
		...
		/**
		 * Checks if a plugin is installed based on its slug.
		 *
		 * @since 2.0.0
		 * @access public
		 * @param string $slug The plugin slug.
		...
		/**
		 * Returns the installation status of a plugin.
		 *
		 * @since 2.0.0
		 * @access public
		 * @param string $slug The plugin slug.
		 * @return string The plugin status (activated, installed, or not_installed).
		 */
		...
		/**
		 * Checks if a plugin is active based on its slug.
		 *
		 * @since 2.0.0
		 * @access public
		 * @param string $slug The plugin slug.
		...
		public function wbcom_enqueue_admin_scripts() {			
			...
			if ( ! wp_script_is( 'wbcom_admin_setting_js', 'enqueued' ) ) {
				...
				wp_register_script(
					...
					BPCHK_PLUGIN_URL . 'admin/wbcom/assets/js/wbcom-admin-setting.js',
					array( 'jquery' ),
					'1.0.0',
					true
				);
		...
		/**
		 * Adds additional pages to the Wbcom plugin admin panel.
		 *
		 * @since 2.0.0
		 * @access public
		 */
		...
		/**
		 * Displays the Wbcom plugins list page.
		 *
		 * @since 2.0.0
		 * @access public
		 */
		...
		/**
		 * Displays the Wbcom themes list page.
		 *
		 * @since 2.0.0
		 * @access public
		 */
		...
		/**
		 * Shortcode for displaying the admin panel's top header navigation.
		 *
		 * @since 2.0.0
		 * @access public
		 */
